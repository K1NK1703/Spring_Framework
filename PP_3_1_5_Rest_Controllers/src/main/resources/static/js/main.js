...
document.addEventListener("DOMContentLoaded", async function header() {
    ...
    let user = await fetch('user/info');
...
document.addEventListener("DOMContentLoaded", async function navigation() {
    ...
    let userResponse = await fetch('user/info');
...
document.addEventListener("DOMContentLoaded", async function panels() {
    ...
    async function populateEditModal(userId) {
        let userResponse = await fetch(`/admin/user?id=${userId}`);
        if (userResponse.ok) {
            let user = await userResponse.json();
            ...
            let rolesResponse = await fetch('admin/roles');
            if (rolesResponse.ok) {
    ...
    async function populateDeleteModal(userId) {
        let response = await fetch(`/admin/user?id=${userId}`);
        if (response.ok) {
            let user = await response.json();
    ...
    async function deleteUser() {
        let response = await fetch('/admin/delete?id=' + userToDeleteId, {
            method: 'DELETE'
        });
